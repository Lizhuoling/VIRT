...
class GripperBoltNut():
    ...
    def init_simulate_env(self, seed = None):
        ...
        # create bolt asset
        bolt_file = "urdf/nut_bolt/bolt_m4_loose_CM.urdf"  #"urdf/nut_bolt/bolt_m4_tight_SI_5x.urdf"
        ...
        bolt_options.linear_damping = 0.5  # default = 0.0
        bolt_options.max_linear_velocity = 1000.0  # default = 1000.0
        bolt_options.angular_damping = 0.5  # default = 0.5
        ...
        bolt_asset = self.gym.load_asset(self.sim, asset_root, bolt_file, bolt_options)

        # create nut asset
        nut_file = "urdf/nut_bolt/nut_m4_loose_CM.urdf"   #"urdf/nut_bolt/nut_m4_tight_SI_5x.urdf"
        ...
        nut_asset = self.gym.load_asset(self.sim, asset_root, nut_file, nut_options)
        ...
        for i in range(num_envs):
            ...
            # add bolt
            bolt_pose.p.x = table_pose.p.x + np.random.uniform(-0.15, -0.05)
            bolt_pose.p.y = table_pose.p.y + np.random.uniform(0.0, 0.04)
            bolt_pose.p.z = table_dims.z
            bolt_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.random.uniform(-math.pi, math.pi))
            bolt_handle = self.gym.create_actor(env, bolt_asset, bolt_pose, "bolt", i, 0)
            bolt_props = self.gym.get_actor_rigid_shape_properties(env, bolt_handle)
            #bolt_props[0].filter = imesh
            bolt_props[0].friction = 1.0  # default = ?
            bolt_props[0].rolling_friction = 1.0  # default = 0.0
            bolt_props[0].torsion_friction = 1.0  # default = 0.0
            bolt_props[0].restitution = 0.0  # default = ?
            bolt_props[0].compliance = 0.0  # default = 0.0
            bolt_props[0].thickness = 0.0  # default = 0.0
            self.gym.set_actor_rigid_shape_properties(env, bolt_handle, bolt_props)

            # get global index of box in rigid body state tensor
            bolt_idx = self.gym.get_actor_rigid_body_index(env, bolt_handle, 0, gymapi.DOMAIN_SIM)
            self.bolt_idxs.append(bolt_idx)

            # add nut
            nut_pose.p.x = bolt_pose.p.x + np.random.uniform(-0.15, -0.05)
            nut_pose.p.y = bolt_pose.p.y + 0.2 + np.random.uniform(0.05, 0.1)
            nut_pose.p.z = table_dims.z + 0.02
            nut_handle = self.gym.create_actor(env, nut_asset, nut_pose, "nut", i, 0)
            nut_props = self.gym.get_actor_rigid_shape_properties(env, nut_handle)
            #nut_props[0].filter = i
            nut_props[0].friction = 1.0  # default = ?
            nut_props[0].rolling_friction = 1.0  # default = 0.0
            nut_props[0].torsion_friction = 1.0  # default = 0.0
            nut_props[0].restitution = 0.0  # default = ?
            nut_props[0].compliance = 0.0  # default = 0.0
            nut_props[0].thickness = 0.0  # default = 0.0
            self.gym.set_actor_rigid_shape_properties(env, nut_handle, nut_props)

            # get global index of box in rigid body state tensor
            nut_idx = self.gym.get_actor_rigid_body_index(env, nut_handle, 0, gymapi.DOMAIN_SIM)
            self.nut_idxs.append(nut_idx)
            ...
            self.task_instruction.append("Please screw the bolt onto the nut.")
