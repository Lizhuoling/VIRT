...
class MultiColorBoxScript():
    def __init__(self, isaac_env):
        ...
        for task_instruction, box_idxs in zip(self.isaac_env.task_instruction, self.isaac_env.box_idxs):
            ...
            self.box1_idx_list.append(box_idxs[box1_key])
            self.box2_idx_list.append(box_idxs[box2_key])

    def get_grasp_pos(self, box_id):
        ...
        if box_id == 1:
            box_pos = self.isaac_env.rb_states[self.box1_idx_list, :3]  # Left shape: (num_hand, 3)
            box_rot = self.isaac_env.rb_states[self.box1_idx_list, 3:7]    # Left shape: (num_hand, 4)
            to_box = box_pos - hand_pos
        elif box_id == 2:
            box_pos = self.isaac_env.rb_states[self.box2_idx_list, :3]
            box_rot = self.isaac_env.rb_states[self.box2_idx_list, 3:7]
            to_box = box_pos - hand_pos
        else:
    ...
    def update_pos_action(self,):
        ...
        box1_pos = self.isaac_env.rb_states[self.box1_idx_list, :3]  # Left shape: (num_hand, 3)
        to_box1 = box1_pos - hand_pos
        box1_dist = torch.norm(to_box1, dim=-1).unsqueeze(-1)

        box2_pos = self.isaac_env.rb_states[self.box2_idx_list, :3]  # Left shape: (num_hand, 3)
        to_box2 = box2_pos - hand_pos
        box2_dist = torch.norm(to_box2, dim=-1).unsqueeze(-1)

        # To the grasp position of box 1
        if self.action_state == 0:
            # goal action
            goal_pos, goal_rot = self.get_grasp_pos(box_id = 1)
            ...
            gripped = (gripper_sep < 0.045) & (box1_dist < self.grasp_offset + 0.5 * self.box_size)

            # gripper action
            if (box1_dist < self.grasp_offset + 0.02) | gripped & torch.logical_not(self.close_gripper):
                self.timer[0] = True
            
            self.close_gripper = (box1_dist < self.grasp_offset + 0.02) | gripped
        ...
        # To the release position of box 1
        elif self.action_state == 2:
            goal_pos = box1_pos.new([[self.isaac_env.container_bottom_pose.p.x, self.isaac_env.container_bottom_pose.p.y - 0.02, self.isaac_env.table_dims.z + 0.5 * self.box_size + self.grasp_offset + 0.05],])
            goal_rot = torch.Tensor([0.7070, 0.7070, 0.015, 0.0]).cuda()[None]
            grip_acts = torch.Tensor([[0., 0.]] * self.isaac_env.num_envs).cuda()
            self.isaac_env.pos_action[:, 7:9] = grip_acts
            if box1_pos[0, 2] <= (self.isaac_env.table_dims.z + 0.5 * self.box_size + 0.2 + 0.01):
                self.timer[0] = True
                self.timer[1] += 1
                if self.timer[1] > 50:
                    self.action_state += 1
        ...
        # To the grasp position of box 2
        elif self.action_state == 5:
            # goal action
            goal_pos, goal_rot = self.get_grasp_pos(box_id = 2)
            # determine if we're holding the box (grippers are closed and box is near)
            gripper_sep = self.isaac_env.dof_pos[:, 7] + self.isaac_env.dof_pos[:, 8]
            gripped = (gripper_sep < 0.045) & (box2_dist < self.grasp_offset + 0.5 * self.box_size)

            # gripper action
            if (box2_dist < self.grasp_offset + 0.02) | gripped & torch.logical_not(self.close_gripper):
                self.timer[0] = True
            
            self.close_gripper = (box2_dist < self.grasp_offset + 0.02) | gripped
            #grip_acts = torch.where(self.close_gripper, torch.Tensor([[0., 0.]] * self.isaac_env.num_envs).cuda(), torch.Tensor([[0.04, 0.04]] * self.isaac_env.num_envs).cuda())
            grip_acts = torch.Tensor([[0.04, 0.04]] * self.isaac_env.num_envs).cuda()
            self.isaac_env.pos_action[:, 7:9] = grip_acts
        ...
        # To the release position of box 2
        elif self.action_state == 7:
            goal_pos = box1_pos.new([[self.isaac_env.container_bottom_pose.p.x, self.isaac_env.container_bottom_pose.p.y - 0.02, self.isaac_env.table_dims.z + 0.5 * self.box_size + self.grasp_offset + 0.07],])
            goal_rot = torch.Tensor([0.7070, 0.7070, 0.015, 0.0]).cuda()[None]
            grip_acts = torch.Tensor([[0., 0.]] * self.isaac_env.num_envs).cuda()
            self.isaac_env.pos_action[:, 7:9] = grip_acts
            if box1_pos[0, 2] <= (self.isaac_env.table_dims.z + 0.5 * self.box_size + 0.2 + 0.01):
