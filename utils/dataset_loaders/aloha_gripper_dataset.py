    ...
    def __getitem__(self, index):
        ...
        with h5py.File(h5py_path, 'r') as root:
            ...
            if start_ts >= (past_obs_len - 1) * obs_sample_interval:
                effort_obs = root['/observations/effort'][start_ts - (past_obs_len - 1) * obs_sample_interval : start_ts + 1 : obs_sample_interval]
                qpos_obs = root['/observations/qpos'][start_ts - (past_obs_len - 1) * obs_sample_interval : start_ts + 1 : obs_sample_interval]
                qvel_obs = root['/observations/qvel'][start_ts - (past_obs_len - 1) * obs_sample_interval : start_ts + 1 : obs_sample_interval]
                observation_len = self.cfg['DATA']['PAST_OBSERVATION_LEN']
            else:
                effort_obs = np.zeros((past_obs_len, root['/observations/effort'][:].shape[1]), np.float32)
                qpos_obs = np.zeros((past_obs_len, root['/observations/qpos'][:].shape[1]), np.float32)
                qvel_obs = np.zeros((past_obs_len, root['/observations/qvel'][:].shape[1]), np.float32)
                valid_past_num = start_ts // obs_sample_interval
                st = start_ts - valid_past_num * obs_sample_interval
                effort_obs[-valid_past_num - 1:] = root['/observations/effort'][st : start_ts + 1 : obs_sample_interval]
                qpos_obs[-valid_past_num - 1:] = root['/observations/qpos'][st : start_ts + 1 : obs_sample_interval]
                qvel_obs[-valid_past_num - 1:] = root['/observations/qvel'][st : start_ts + 1 : obs_sample_interval]
        ...
        effort_obs = (effort_obs - self.norm_stats['observations/effort_mean']) / self.norm_stats['observations/effort_std']
        qpos_obs = (qpos_obs - self.norm_stats['observations/qpos_mean']) / self.norm_stats['observations/qpos_std']
        qvel_obs = (qvel_obs - self.norm_stats['observations/qvel_mean']) / self.norm_stats['observations/qvel_std']

        image_data = self.transforms(image_data)
        
        return image_data, past_action, action_data, effort_obs, qpos_obs, qvel_obs, observation_is_pad, past_action_is_pad, action_is_pad, task_instruction, status
